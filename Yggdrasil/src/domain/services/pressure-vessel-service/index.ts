...
class PressureVesselService {
    ...
    async isFillingAllowed(pressureVessel: PressureVessel): Promise<boolean> {
        const pressureBelowMaximum =
            pressureVessel.pressurePsi <= pressureVessel.maximumPressurePsi
        const pressureRecent =
            Date.now() - pressureVessel.lastReadingDate.getTime() <
            pressureVessel.maxReadingDateAgeMs
