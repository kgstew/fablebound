...
const startMidiServer = async () => {
    ...
    if (portCount === 0) {
        console.log(
            'No MIDI input ports available. Please set up a virtual MIDI port.'
        )
    }
    ...
    function signalPixelblaze(
        pixelBlazeNumber: number,
        newSegment?: number,
        lightning = false
    ) {
        var params = {}
        ...
        if (
            pixelBlazeNumber >= 0 &&
            pixelBlazeNumber < pixelblazeClients.length
        ) {
            ...
            if (targetClient.isConnected()) {
                ...
                console.log(
                    `Sent pattern to Pixelblaze ${pixelBlazeNumber}, ${params}`
                )
            } else {
                console.log(
                    `Cannot send pattern to Pixelblaze ${pixelBlazeNumber}: not connected`
                )
            }
        } else {
            console.log(
                `Pixelblaze ${pixelBlazeNumber} not found in the client list`
            )
        }
    }
    ...
    input.on('message', (deltaTime: number, message: number[]) => {
        ...
        PneumaticsModelSingleton.getInstance().model.handleCommand(
            commandMessage
        )
        const params = {
            TRIGGER_SEGMENT_CHANGE: 1,
        ...
        pixelblazeClients.forEach((client) => {
            if (client.isConnected()) {
                ...
                console.log(
                    `Cannot send pattern to ${client.name}: not connected`
                )
            }
        })
    })
