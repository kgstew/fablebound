...
export class PixelblazeClient {
    ...
    connect(): void {
        ...
        this.ws.on('close', (code: number, reason: string) => {
            console.log(`Disconnected from Pixelblaze ${this.name}: ${code} - ${reason}`);
            this.stopHeartbeat();
    ...
    private startHeartbeat(): void {
        ...
        this.heartbeatInterval = setInterval(() => {
            ...
            if (timeSinceLastMessage > this.heartbeatTimeout) {
                console.log(`No message received from ${this.name} in ${this.heartbeatTimeout / 1000} seconds. Reconnecting...`);
                this.reconnect();
            }
    ...
    private scheduleReconnect(): void {
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
            console.log(`Max reconnect attempts reached for Pixelblaze ${this.name}. Giving up.`);
            return;
        }
        ...
        console.log(`Scheduling reconnection to Pixelblaze ${this.name} in ${delay}ms`);
...
export const startPixelblazeClients = () => {
    pixelblazeClients = Object.entries(appConfig.pixelblazes).map(([name, url]) => {
        const client = new PixelblazeClient(name, url);
        client.connect();
        return client;
    });

    console.log('🚀 Pixelblaze clients initialized:', pixelblazeClients.map(client => client.name).join(', '));
};