...
abstract class ReadingsHandlerBase implements Handler<BowOrSternReadingsData> {
    ...
    validate(data: unknown): BowOrSternReadingsData {
        ...
        // console.log(cmd)
        if (
            (cmd.type !== 'espToServerSystemStateBow' &&
                cmd.type !== 'espToServerSystemStateStern') ||
    ...
    private validateBigAssMainTankReadings(
        tank: BigAssMainTankReadings | undefined
    ): asserts tank is BigAssMainTankReadings {
        if (
            !tank ||
            typeof tank.pressurePsi !== 'number' ||
            !this.validateValve(tank.compressorToTankValve)
        ) {
            throw new Error('Invalid BigAssMainTank readings')
        }
    }

    private validateLegAssemblyReadings(
        assembly: LegAssemblyReadings | undefined,
        name: string
    ): asserts assembly is LegAssemblyReadings {
        if (
            !assembly ||
            typeof assembly.ballastPressurePsi !== 'number' ||
    ...
    async handle(data: unknown): Promise<void> {
        ...
        pneumaticsModelSingleton.model.updateSystemStateFromReadings(
            validatedData
        )
